...
public class AuthenticationFilter implements Filter, Serializable {
	...
	public static final String PATH_LOGIN = "/pages/login.html";
	public static final String PATH_ALLUNPAID = "/pages/allUsersUnpaidBills.html";
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		 ...
		 * the user is redirected to login screen (if requestedPath == index
		 ...
		 * user, but the user is trying to log in (if requestedPath == login
		 * continue to next filter)
		 */
		...
		if (PATH_ALLUNPAID.equals(requestedPath)) {
			chain.doFilter(request, response);
			return;
		}
